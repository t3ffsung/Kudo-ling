...
function App() {
  ...
  // Direct challenge function for the friends list
  const challengeFriend = async (friendUid, friendName) => {
    ...
    const initialTokens = { red: BASE_POSITIONS.red, yellow: BASE_POSITIONS.yellow };
    const playersInfo = {
      red: { name: profile.displayName, uid: user.uid, isBot: false },
      yellow: { name: "Waiting...", isBot: false }
    };
    ...
    try {
      ...
      await update(ref(db, `rooms/${code}`), {
        ...
        payoutClaimed: false, pot: BET_AMOUNT, scores: { red: 0, green: 0, yellow: 0, blue: 0 }
      });
      // Automatically send the invite to the friend
      ...
      setAlertMsg(`Invited ${friendName}! Waiting in lobby...`);
    ...
    return (
      <div className="home-container">
        ...
        <div className="home-actions">
          ...
          <button className="btn-huge btn-friends" style={{background: '#a855f7'}} onClick={() => setActiveModal('friends')}>
            ...
            <div className="modal-content profile-modal-content">
              ...
              <div style={{width:'100%', display:'flex', gap:'10px', marginBottom:'20px'}}>
                 ...
                 <button className="btn btn-primary" style={{margin:0}} onClick={() => {
                   setAlertMsg("Ensure the Friend Code matches exactly to add.");
                 }}>Add</button>
              </div>
              ...
              <div className="leaderboard-list">
                {leaderboard.map((u, i) => {
                  ...
                  <div key={i} className={`lb-item ${isMe ? 'is-me' : ''}`}>
                    ...
                    {!isMe && isFriend && (
                       <span style={{fontSize:'12px', color:'#22c55e', fontWeight:'bold', marginRight:'5px'}}>✓ Added</span>
                    )}
                  </div>
                )})}
              </div>
